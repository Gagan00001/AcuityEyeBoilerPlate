...
import AntdAutocomplete from '../index';
...
const defaultProps = {
  ...
  debounceDuration: 0,
  dataTestId: 'provider-customAutoComplete',
};
...
test('should render custom autoComplete component', () => {
  ...
  const autoCompleteInput = screen.getByTestId('provider-customAutoComplete');
...
test('should be able to search and select value from dropDown list after API call', async () => {
  ...
  const autoCompleteSelect = screen.getByTestId('provider-customAutoComplete').querySelector('.ant-select-selection-search-input');
  ...
  const selectedValue = screen.getByTestId('provider-customAutoComplete').querySelector('.ant-select-selection-item');
  expect(selectedValue).toHaveTextContent('AJ Provider');
});
