...
const actionsMap = {
  [SET_DATA_READ]: (state, { id, data, params }) => {
    if (params) {
      return state.setIn([id, 'read', 'data'], data).setIn([id, 'read', 'loading'], false).setIn([id, 'read', 'params'], params);
    }
    return state.setIn([id, 'read', 'data'], data).setIn([id, 'read', 'loading'], false);
  },
  [SET_LOADING_READ]: (state, { id, loading, params }) => state.setIn([id, 'read', 'loading'], loading).setIn([id, 'read', 'params'], params),
...
export default function crudWithoutTab(state = initialState, action = {}) {
