...
import {
  ...
  CLEAR_READ_DATA,
  SET_DATA_UPDATE_ALL,
  SET_LOADING_UPDATE_ALL,
  SET_ERROR_UPDATE_ALL,
} from '../actions/crud';
...
const actionsMap = {
  ...
  [SET_DATA_UPDATE_ALL]: (state, { id, data }) => state.setIn([id, 'updateAll', 'data'], data),
  [SET_LOADING_UPDATE_ALL]: (state, { id, loading }) => state.setIn([id, 'updateAll', 'loading'], loading),
  [SET_ERROR_UPDATE_ALL]: (state, { id, error }) => state.setIn([id, 'updateAll', 'error'], error),
  [CLEAR_DATA]: (state, { id }) => state.deleteIn([id, 'updateAll']).deleteIn([id, 'update']).deleteIn([id, 'create']).deleteIn([id, 'delete']),
